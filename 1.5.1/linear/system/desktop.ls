...
local function showTime()
 local time1 = os.time()
 if timeFormat == "12HR" then
   local time2 = textutils.formatTime(time1,false)
    bCol(colors.white)
	tCol(colors.black)
    clear(1,7)
    centerPrint("The time is "..time2)
    sleep(1)
	return
    runDesktop()
  elseif timeFormat == "24HR" then
   local time2 = textutils.formatTime(time1,true)
    bCol(colors.white)
	tCol(colors.black)
    clear(1,7)
    centerPrint("The time is "..time2)
...
---------------------------------------------------------------------------------------------------------------------------------
                                            ...
                                            --[[ CODE ]]--
if fs.exists("linear/userfolders/"..user.getCurrent().."/desktopConfig.ls") then
 ...
 while true do
  ...
  if event == "mouse_click" then 
   if x>2 and x<6 and y==1 and button ==1 then 
    return
    drawMainMenu()
   elseif x>44 and x<52 and y==1 and button==1 then
    return
    showTime()
   else
   end
...
---------------------------------------------------------------------------------------------------------------------------------
  ...
  local event, button, x, y = os.pullEventRaw()
  if event == "mouse_click" then 
   if x>1 and x<52 and y==8 and button==1 then
	target1.mBarC = 2048
	step2()
   elseif x>1 and x<52 and y==9 and button==1 then
	target1.mBarC = 16384
	step2()
   elseif x>1 and x<52 and y==10 and button==1 then
	target1.mBarC = 8192
	step2()
   elseif x>1 and x<52 and y==11 and button==1 then
	target1.mBarC = 512
	step2()
